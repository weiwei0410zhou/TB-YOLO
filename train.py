...
def train(yaml_path):
    ...
    yolo = YOLO('/home/zww/2t_ssd1_ww/zww/yolov10-SEAttention/ultralytics/cfg/Ablation experiment_v10s/Detect_FASFF/yolov10s-SEAttention-C3K2-C2f_EMA.yaml')
...
def predict(model_path):
    ...
    # 使用多进程并行处理图像
    with ProcessPoolExecutor(max_workers=8) as executor:  # 增加最大并行进程数到8
...
def process_sub_img(model, sub_img, x_offset, y_offset, sub_img_save_dir):
    sub_img_path = os.path.join(sub_img_save_dir, f'sub_img_{x_offset}_{y_offset}.png')
...
def predict_imgs_big(model_path, large_img_dir, output_dir):
    ...
    for large_img_path in large_img_paths:
        # 为每张大图创建一个单独的文件夹来保存对应的小图
        large_img_name = os.path.splitext(os.path.basename(large_img_path))[0]
        sub_img_save_dir = os.path.join(output_dir, 'sub_imgs', large_img_name)
        os.makedirs(sub_img_save_dir, exist_ok=True)
        ...
        for y in range(0, img_height, stride):
            for x in range(0, img_width, stride):
                ...
                if sub_img_width < 1280 or sub_img_height < 1280:
                    continue  # 跳过小于1280大小的子图
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(process_sub_img, model, sub_img, x_offset, y_offset, sub_img_save_dir) for sub_img, x_offset, y_offset in sub_imgs]
        ...
        for i in range(len(final_boxes)):
            ...
            draw.rectangle([x1, y1, x2, y2], outline="red", width=4)  # 增加边框厚度
...
if __name__ == '__main__':
    ...
    #train('data.yaml')
    #predict_imgs_big('runs/detect/Ablation experiment_v10s/dataset_all_models/train/train_yolov10s-SEA-Detect_FASFF-C3K2_2-C2f_EMA4/weights/best.pt', '/home/zww/8t_sdc/zww/demo/qupath', '/home/zww/8t_sdc/zww/demo/qupath_test')
